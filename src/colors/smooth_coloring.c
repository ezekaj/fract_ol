...
/*   Updated: 2025/05/18 23:45:45 by ezekaj           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	smooth_color_blue_animated(double smooth_iter, int max_i, double time)
{
	...
	color.g = (int)(120 + 80
			* sin(color.t * 6.0 + 1.0 + color.time_phase * 0.7));
...
int	psychedelic_animated(double smooth_iter, int max_i, double time)
{
	...
	color.r = (int)(127.5 + 127.5 * sin(color.phase));
	color.g = (int)(127.5 + 127.5
			* sin(color.phase + 2.094 + sin(color.time_phase * 0.3) * 0.5));
	color.b = (int)(127.5 + 127.5
			* sin(color.phase + 4.188 + cos(color.time_phase * 0.2) * 0.5));
	color.r = (int)(color.r * (0.8 + 0.2
				* sin(color.t * 5.0 + color.time_phase)));
	color.g = (int)(color.g * (0.8 + 0.2
				* cos(color.t * 7.0 + color.time_phase * 0.7)));
	color.b = (int)(color.b * (0.8 + 0.2
				* sin(color.t * 11.0 + color.time_phase * 0.5)));
	if (color.r < 0)
		color.r = 0;
	else if (color.r > 255)
		color.r = 255;
...
int	smooth_color_grayscale(double smooth_iter, int max_i)
{
	...
	return ((color.brightness << 24)
		| (color.brightness << 16) | (color.brightness << 8) | 0xFF);
}
...
int	smooth_color_grayscale_animated(double smooth_iter, int max_i, double time)
{
	...
	color.t = smooth_iter / max_i;
	color.time_phase = time * 0.3;
	color.brightness = (int)(255 * (0.5 + 0.5
				* cos(color.t * 10.0 + color.time_phase)));
	if (color.brightness < 0)
		color.brightness = 0;
	else if (color.brightness > 255)
		color.brightness = 255;
